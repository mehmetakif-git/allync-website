...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  if (!webGLSupported) {
    return (
      <div
        className={`w-full h-full fixed inset-0 flex items-center justify-center bg-black/10 text-gray-500 text-sm ${className}`}
        style={{ mixBlendMode, zIndex: 0 }}
      >
        WebGL not supported
      </div>
    );
  ...
  return (
    <div ref={containerRef} className={`w-full h-full fixed inset-0 ${className}`} style={{ mixBlendMode, zIndex: 0 }} />
  );
};
